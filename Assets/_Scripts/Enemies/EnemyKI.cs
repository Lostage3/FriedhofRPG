...
public class EnemyKI : MonoBehaviour
{
    ...
    [SerializeField] float xWalkDistance = 30f;
    ...
    private void Awake()
    {
        ...
        wayPoint2 = new Vector3(transform.position.x + xWalkDistance, transform.position.y, transform.position.z);
    ...
    private void Update()
    {
        ...
        if (transform.position.x >= wayPoint2.x - 5f)
        {
            agent.SetDestination(wayPoint1);
        }
        else if (transform.position.x <= wayPoint1.x + 5f)
        {   
            agent.SetDestination(wayPoint2);
        }
